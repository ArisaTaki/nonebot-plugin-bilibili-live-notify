...
async def test_store_roundtrip(tmp_path: Path):
    ...
    await store.subscribe_group(room_id=123, remark="测试主播", group_id=456, creator_id=789)
    assert await store.add_participant(123, 456, 10001) is True
...
@pytest.mark.asyncio
async def test_unsubscribe_last_group_removes_room(tmp_path: Path):
    store = Store(tmp_path / "subscriptions.json")
    await store.subscribe_group(room_id=123, remark="测试主播", group_id=456, creator_id=789)

    assert await store.unsubscribe_group(123, 456) is True
